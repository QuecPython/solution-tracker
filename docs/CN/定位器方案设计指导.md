...
- Server 为服务层, 对讲不同的IoT 平台, 使用不同的协议进行交互
- Tracker 为核心业务功能, 主要用于采集设备数据, 控制设备模块, 接收处理IoT 平台下行指令, 数据的整合上报
- Modules 为独立功能模块, 主要是定位数据的获取与解析, 传感器数据的读取, 电池管理, 历史数据存储等功能
- Device 主要为设备数据的信息的采集与设备功能设置, 如设备的版本信息, IMEI号, APN设置, 网络拨号, 设备低功耗等

![solution-tracker-103](./media/solution-tracker-103.png)
